...
## at that tier (demotes at 3), a title count per Grand Slam, and - so a
## closed app or an early exit doesn't lose bracket progress - which
## tournament/round was in progress, if any. Saved as plain JSON to
## user://career_save.json - no engine-specific resource formats, so it
## stays easy to read/reset by hand if ever needed. Written after every
## result, so "closed the app" and "explicitly went back to the menu" behave
## identically - there's never an unsaved window.
...
## the counter - the same rule applies even at the Grand Slams.
