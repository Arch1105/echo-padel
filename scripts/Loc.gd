...
const STRINGS := {
	...
	"help_wall_mode_body": {"en": "A third online option, next to Online Mode and Quick Online Mode, connected the same way (Create Room or Join Room). The court's side walls are in play - a shot spun hard enough to normally sail out to the side instead bounces off that wall with a distinct thump and carries on into play, landing on the opposite side of the court instead of faulting. To help you get across in time, press D-pad Left/Right on a controller, or hold F and press the Left/Right arrow key, to jump straight to that side's edge tile, skipping the middle. You also get one Super Smash per game: press D-pad Up (or J) right after you hear the ball's first bounce on your side, then press the smash button before the second bounce - miss either half of that timing and it just doesn't arm, no penalty. Land it and the ball crosses much faster than a normal smash. Scoring works differently here too: points race up with no cap, first to five - but reaching five doesn't win by itself, it just makes you eligible. The match only ends when a player who's already past five wins a point specifically with a smash; winning a normal point past five just keeps the match going.",
			"es": "Una tercera opción en línea, junto a Modo en línea y Modo rápido en línea, con la misma forma de conectar (Crear sala o Unirse a sala). Las paredes laterales de la pista están en juego - un golpe con suficiente efecto como para salir normalmente por el lateral en su lugar rebota en esa pared con un golpe seco distintivo y sigue en juego, cayendo en el lado opuesto de la pista en vez de ser falta. Para ayudarte a llegar a tiempo, pulsa D-pad izquierda/derecha en un mando, o mantén pulsada F y pulsa la flecha izquierda/derecha, para saltar directamente a la casilla del borde de ese lado, saltándote el centro. También tienes un Súper remate por partida: pulsa D-pad arriba (o J) justo después de escuchar el primer bote de la pelota en tu lado, y luego pulsa el botón de remate antes del segundo bote - si fallas cualquiera de los dos tiempos, simplemente no se activa, sin penalización. Si lo consigues, la pelota cruza mucho más rápido que un remate normal. La puntuación también funciona distinto aquí: los puntos suben sin límite, el primero en llegar a cinco - pero llegar a cinco no gana por sí solo, solo te hace elegible. La partida solo termina cuando un jugador que ya superó cinco gana un punto específicamente con un remate; ganar un punto normal después de cinco simplemente continúa la partida."},
	...
	"wall_online_mode_desc": {"en": "The court's side walls are in play - a shot that would go out bounces off the wall into play on the opposite side instead. First to five points, then win a point with a smash to win the match. See Help for the side-dash and Super Smash moves this mode adds.",
			"es": "Las paredes laterales de la pista están en juego - un golpe que saldría fuera rebota en la pared y sigue en juego en el lado opuesto. El primero en llegar a cinco puntos y luego ganar un punto con un remate gana el partido. Consulta la Ayuda para ver los movimientos de esquive lateral y Súper remate que añade este modo."},
